...
const analyze = (text, languageId = 'ie-lua') =>
  analyzeDocument({ uri: 'file:///binding.lua', text, languageId, luaparse });
...
test('lexical bindings separate shadowed variables and exclude comments, strings, and fields', () => {
  ...
  assert.deepEqual(
    renameLocations(a, inner, 'inner').map((l) => l.offsetRange.start),
    [inner, text.indexOf('x)')],
  );
  assert.deepEqual(
    renameLocations(a, text.indexOf('x = 1'), 'outer').map((l) => l.offsetRange.start),
...
test('initializers, recursion, closure capture, loop scopes and repeat conditions resolve correctly', () => {
  ...
  assert.equal(
    visibleSymbols(a, text.length).some((s) => s.name === 'i' || s.name === 'ready'),
    false,
  );
});
...
test('incomplete buffers preserve discovery without allowing guessed rename', () => {
  ...
  assert.equal(
    a.symbols.some((s) => s.name === 'fake'),
    false,
  );
  assert.equal(
    a.references.some((r) => r.name === 'ghost'),
    false,
  );
...
test('menu analysis excludes DSL and isolates locals while sharing document globals', () => {
  const text =
    'menu { text "local fake = 1" action "local x = 1; print(x)" onOpen "local x = 2; print(x); shared()" }\n`function shared() end`';
  const a = analyze(text, 'ie-menu');
  assert.equal(a.diagnostics.length, 0);
  assert.equal(a.embeddedRegions.length, 3);
  assert.equal(
    a.symbols.some((s) => s.name === 'fake' || s.name === 'menu'),
    false,
  );
  ...
  assert.deepEqual(
    locations.map((l) => l.offsetRange.start),
