...
function validateRelease(type, tag, version) {
  ...
  assert.match(tag, new RegExp(`^v${version.replaceAll('.', '\\.')}${suffix}$`, 'u'), 'Tag must match VERSION and channel');
...
if (require.main === module) {
  validateRelease(process.env.RELEASE_TYPE, process.env.TAG_NAME, fs.readFileSync('VERSION', 'utf8').trim());
}
