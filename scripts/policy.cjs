...
function validateLabels(definitions, rules) {
  ...
  for (const name of Object.keys(rules)) assert.ok(definitions.some((d) => d.name === name), `Missing label definition: ${name}`);
}
function validateVersions(version, pkg, lock, workspaces, changelog) {
  ...
  assert.equal(lock.packages[''].devDependencies['@types/vscode'], pkg.devDependencies['@types/vscode']);
  assert.equal(lock.packages['node_modules/@types/vscode'].version, pkg.devDependencies['@types/vscode']);
}
    ...
    for (const job of Object.values(workflow.jobs)) {
      ...
      for (const step of job.steps ?? []) {
        if (step.uses && !step.uses.startsWith('./')) {
          ...
          if (step.uses.startsWith('github/codeql-action/')) codeqlPins.add(step.uses.split('@')[1]);
        }
        if (workflow.on?.pull_request_target) {
          assert.ok(!step.uses?.startsWith('actions/checkout@'), `${file}: no checkout in privileged PR workflow`);
        }
      }
    }
  }
...
function main() {
  ...
  validateVersions(fs.readFileSync('VERSION', 'utf8').trim(), pkg, json('package-lock.json'), pkg.workspaces.map((directory) => ({directory, manifest: json(`${directory}/package.json`)})), fs.readFileSync('CHANGELOG.md', 'utf8'));
  validateLabels(json('.github/labels.json'), readYaml('.github/labeler.yml'));
  validateWorkflows(fs.readdirSync('.github/workflows').filter((f) => f.endsWith('.yml')).map((f) => [f, readYaml(`.github/workflows/${f}`)]));
  for (const file of ['SECURITY.md', 'docs/architecture/THREAT_MODEL.md', 'docs/release/READINESS.md']) assert.ok(fs.existsSync(file), file);
  ...
  for (const config of json('.vscode/launch.json').configurations) assert.equal(config.preLaunchTask, undefined);
  ...
  for (const language of ['ie-lua', 'ie-menu']) assert.equal(kate.servers[language].command.at(-1), '--stdio');
  for (const directory of ['.github', 'packages', 'resources/api']) assert.ok(fs.existsSync(path.join(directory, 'AGENTS.md')));
