...
async function fixture(t, change) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'ie-vsix-'));
  ...
  const files = {
    'extension.vsixmanifest':
      '<PackageManifest><Property Id="Microsoft.VisualStudio.Code.PreRelease" Value="true" /></PackageManifest>',
  ...
  for (const [name, value] of Object.entries(files)) {
    ...
    if (name.startsWith('extension/')) {
      const target = path.join(root, name.slice(10));
      fs.mkdirSync(path.dirname(target), { recursive: true });
      fs.writeFileSync(target, value);
    }
  }
  if (change) change(zip);
  const file = path.join(root, 'test.vsix');
  fs.writeFileSync(file, await zip.generateAsync({ type: 'nodebuffer' }));
  return { file, root };
}
test('archive audit checks shipped bytes and channel', async (t) => {
  const { file, root } = await fixture(t);
  await auditVsix(file, 'prerelease', root);
  await assert.rejects(auditVsix(file, 'stable', root), /channel/u);
});
for (const [name, change, error] of [
  ...
  test(`archive audit rejects ${name}`, async (t) => {
    const { file, root } = await fixture(t, change);
    await assert.rejects(auditVsix(file, 'prerelease', root), error);
  });
