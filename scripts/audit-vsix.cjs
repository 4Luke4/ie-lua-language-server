...
async function auditVsix(file, expectedChannel, root = process.cwd()) {
  ...
  const names = Object.values(zip.files)
    ...
    .map((f) => {
      ...
      assert.ok(
        !f.name.includes('\\') && !f.name.startsWith('/') && !f.name.split('/').includes('..'),
        'Unsafe archive path',
      );
      return f.name;
    });
  const required = [
    ...
    'extension/resources/api/api-index.json',
  ...
  for (const name of names) {
    assert.ok(
      !/(^|\/)(node_modules|packages|scripts|tests|samples|\.git|\.github|\.codex|\.agents|\.vscode|reports|artifacts)(\/|$)/u.test(
        name,
      ),
      `Development content: ${name}`,
    );
    assert.ok(
  ...
  assert.equal(
    /Id="Microsoft\.VisualStudio\.Code\.PreRelease"\s+Value="true"/u.test(manifest),
    expectedChannel === 'prerelease',
    'Incorrect VSIX channel',
  );
  const index = JSON.parse(
    await zip.file('extension/resources/api/api-index.json').async('string'),
  );
  const shards = index.sections.flatMap((section) =>
    section.files.map((ref) => `extension/resources/api/${ref.file}`),
  );
  for (const name of [
    ...shards,
    ...pkg.contributes.grammars.map((g) => `extension/${g.path.replace(/^\.\//u, '')}`),
    `extension/${pkg.icon}`,
  ])
    assert.ok(names.includes(name), `Missing asset: ${name}`);
  for (const name of names.filter((n) => n.startsWith('extension/resources/api/sections/')))
    assert.ok(shards.includes(name), `Orphan shard: ${name}`);
  // Compare actual shipped bytes, not merely the packaging file list.
  for (const name of names.filter(
    (n) => n.startsWith('extension/resources/') || n.startsWith('extension/dist/'),
  )) {
    assert.deepEqual(
      await zip.file(name).async('nodebuffer'),
      fs.readFileSync(path.join(root, name.slice(10))),
      `Archive differs from audited source: ${name}`,
    );
  }
...
if (require.main === module) {
  auditVsix(process.argv[2], process.argv[3])
    .then(console.log)
    .catch((error) => {
      console.error(error);
      process.exitCode = 1;
