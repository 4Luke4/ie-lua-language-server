...
export type BindingAnalysis = Pick<AnalyzedDocument, 'symbols' | 'references' | 'folds' | 'semanticTokens'>;
const node = (value: unknown): Node | undefined =>
  value && typeof value === 'object' && 'type' in value && 'range' in value ? value as Node : undefined;
const nodes = (value: unknown): Node[] => Array.isArray(value) ? value.flatMap(v => node(v) ?? []) : [];

export function analyzeBindings(text: string, ast: unknown): BindingAnalysis {
  const symbols: SymbolInfo[] = [], references: ReferenceInfo[] = [];
  ...
  function reference(n: Node, scope: Scope, declaration?: SymbolInfo) {
    ...
    references.push({ name, location: location(n), ...(resolved ? { resolvedDeclaration: resolved } : {}),
      ...(declaration ? { isDeclaration: true } : {}) });
  }
  function declare(n: Node, scope: Scope, kind: SymbolInfo['kind'], local: boolean, visibleFrom: number) {
    const name = String(n.name);
    let symbol = local ? undefined : globals.get(name);
    if (!symbol) {
      symbol = { name, kind, location: location(n), bindingId: local ? `local:${n.range[0]}` : `global:${name}`,
        scopeRange: local ? scope.range : root.range, scopeDepth: local ? scope.depth + 1 : 0,
  ...
  function scoped(n: Node, parent: Scope): Scope {
    return { parent, range: { start: n.range[0], end: n.range[1] }, depth: parent.depth + 1, bindings: new Map() };
  }
  ...
  function visit(value: unknown, scope: Scope) {
    if (Array.isArray(value)) { for (const item of value) visit(item, scope); return; }
    const n = node(value);
    if (!n) return;
    const body = nodes(n.body);
    switch (n.type) {
      case 'Identifier': reference(n, scope); return;
      case 'LocalStatement':
        // Lua locals become visible after their initializers, including multi-assignment.
        visit(n.init, scope);
        for (const variable of nodes(n.variables)) declare(variable, scope, 'local', true, n.range[1]);
        return;
      case 'FunctionDeclaration': {
        ...
        if (identifier?.type === 'Identifier') {
          if (n.isLocal) declare(identifier, scope, 'function', true, identifier.range[1]);
            ...
            if (existing) { existing.kind = 'function'; reference(identifier, scope); }
        ...
        for (const parameter of nodes(n.parameters)) {
          if (parameter.type === 'Identifier') declare(parameter, inner, 'parameter', true, parameter.range[1]);
        }
        // Colon methods have an implicit local receiver with no editable source declaration.
        if (identifier?.indexer === ':') inner.bindings.set('self', {
          name: 'self', kind: 'parameter', location: location(identifier), bindingId: `implicit:${n.range[0]}`,
          scopeRange: inner.range, scopeDepth: inner.depth + 1, visibleFrom: identifier.range[1],
        });
        visit(body, inner); fold(n); return;
      }
      case 'AssignmentStatement':
        visit(n.init, scope);
        for (const variable of nodes(n.variables)) {
          if (variable.type === 'Identifier' && !lookup(scope, String(variable.name))) declare(variable, scope, 'global', false, 0);
          else visit(variable, scope);
        }
        return;
      case 'ForNumericStatement':
      case 'ForGenericStatement': {
        visit(n.start, scope); visit(n.end, scope); visit(n.step, scope); visit(n.iterators, scope);
        const inner = scoped(n, scope);
        const variables = n.type === 'ForNumericStatement' ? [node(n.variable)].filter((v): v is Node => !!v) : nodes(n.variables);
        for (const variable of variables) declare(variable, inner, 'local', true, body[0]?.range[0] ?? n.range[1]);
        visit(body, inner); fold(n); return;
      }
      case 'RepeatStatement': {
        const inner = scoped(n, scope); visit(body, inner); visit(n.condition, inner); fold(n); return;
      }
      case 'IfClause': case 'ElseifClause': case 'ElseClause': case 'WhileStatement': case 'DoStatement':
        visit(n.condition, scope); visit(body, scoped(n, scope)); fold(n); return;
      case 'MemberExpression':
        // Dot/colon field names are not references to lexically scoped variables.
        visit(n.base, scope); return;
      case 'TableKeyString': visit(n.value, scope); return;
      case 'LabelStatement': case 'GotoStatement': return;
      case 'Comment': fold(n, 'comment'); return;
      default:
  ...
  const semanticTokens = references.flatMap(ref => {
    ...
    if (!symbol) return [];
    return [{ location: ref.location,
      tokenType: symbol.kind === 'parameter' ? 'parameter' as const : symbol.kind === 'function' ? 'function' as const : 'variable' as const,
...
export function visibleSymbols(analysis: AnalyzedDocument, offset: number): SymbolInfo[] {
  const visible = analysis.symbols.filter(s => !s.scopeRange ||
    (s.scopeRange.start <= offset && offset <= s.scopeRange.end && (s.visibleFrom ?? 0) <= offset));
  visible.sort((a, b) => (b.scopeDepth ?? 0) - (a.scopeDepth ?? 0) || (b.visibleFrom ?? 0) - (a.visibleFrom ?? 0));
...
export function referenceAt(analysis: AnalyzedDocument, offset: number): ReferenceInfo | undefined {
  return analysis.references.find(r => r.location.offsetRange.start <= offset && offset < r.location.offsetRange.end);
}
...
export function renameLocations(analysis: AnalyzedDocument, offset: number, newName: string) {
  ...
  if (!analysis.bindingsComplete || !target?.bindingId || target.bindingId.startsWith('implicit:')) return undefined;
  const renamed = { ...analysis, symbols: analysis.symbols.map(s => s.bindingId === target.bindingId ? { ...s, name: newName } : s) };
  ...
  for (const ref of analysis.references) {
    ...
    const resolved = visibleSymbols(renamed, ref.location.offsetRange.start).find(s => s.name === newName);
  ...
  if (analysis.symbols.some(s => s.bindingId !== target.bindingId && s.name === newName &&
    s.scopeRange?.start === target.scopeRange?.start && s.scopeRange?.end === target.scopeRange?.end)) return undefined;
  return analysis.references.filter(r => r.resolvedDeclaration?.bindingId === target.bindingId).map(r => r.location);
}
...
export function maskLuaTrivia(text: string): string {
  ...
  let i = 0;
  const hide = (start: number, end: number) => { for (let j = start; j < end; j++) if (chars[j] !== '\n' && chars[j] !== '\r') chars[j] = ' '; };
  while (i < text.length) {
    const start = i;
    ...
    if (long) {
      ...
      i = end < 0 ? text.length : end + close.length; hide(start, i); continue;
    }
    if (comment) { const end = text.indexOf('\n', i); i = end < 0 ? text.length : end; hide(start, i); continue; }
    if (text[i] === '"' || text[i] === "'") {
      const quote = text[i++];
      while (i < text.length) { if (text[i] === '\\') { i += 2; continue; } if (text[i++] === quote) break; }
      hide(start, Math.min(i, text.length)); continue;
    }
    i++;
  }
  return chars.join('');
