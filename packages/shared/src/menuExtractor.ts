...
export function extractEmbeddedLua(text: string, settings: IeLuaSettings): EmbeddedLuaRegion[] {
  ...
  const blocks = new Set(
    settings.menu.blockKeys.map((key) => key.toLowerCase().replace(/\s+/gu, ' ')),
  );
  const expressions = new Set(settings.menu.expressionKeys.map((key) => key.toLowerCase()));
  let i = 0;
  const add = (
    quoted: QuotedString,
    kind: EmbeddedLuaRegion['kind'],
    start: number,
    key?: string,
  ) => {
    if (!quoted.content.trim()) return;
    regions.push({
      id: `menu-${regions.length}`,
      kind,
      ...(key ? { key } : {}),
      luaText: quoted.content,
      hostRange: { start, end: quoted.range.end },
      luaContentRange: quoted.contentRange,
      virtualPrefix: kind === 'expression' ? 'return ' : '',
      virtualSuffix: '',
    });
  };
  while (i < text.length) {
    ...
    if (text[i] === '"') {
      i = readQuotedString(text, i)!.range.end;
      continue;
    }
    ...
    const quoted = readQuotedString(text, i);
    if (!quoted) continue;
    if (explicit || blocks.has(key) || expressions.has(key))
      add(quoted, explicit || expressions.has(key) ? 'expression' : 'block', start, key);
    i = quoted.range.end;
  }
  return regions;
}
...
function readQuotedString(text: string, start: number, delimiter = '"'): QuotedString | undefined {
  ...
  return {
    ...
    range: { start, end: Math.min(end + 1, text.length) },
    contentRange: { start: start + 1, end },
  };
}
